...
Appareil rechercher_appareil(const char *piece);
...
void *thread_serveur_chauffage(void *arg)
{
    ...
    // Configuration de l'adresse locale pour écouter les commandes UDP
    struct sockaddr_in udp_addr;
    memset(&udp_addr, 0, sizeof(udp_addr));
    udp_addr.sin_family = AF_INET;
    udp_addr.sin_port = htons(CENTRAL_SYSTEM_PORT);
    udp_addr.sin_addr.s_addr = htonl(INADDR_ANY);

    if (bind(udp_socket, (struct sockaddr *)&udp_addr, sizeof(udp_addr)) == SOCKET_ERROR)
    {
        fprintf(stderr, "Erreur bind socket UDP : %d\n", WSAGetLastError());
        closesocket(client_socket);
        closesocket(udp_socket);
        return NULL;
    }

    printf("En attente de commandes de chauffages");

    struct sockaddr_in sender_addr;
    int sender_addr_len = sizeof(sender_addr);

    while ((recvlen = recvfrom(udp_socket, buffer, sizeof(buffer) - 1, 0, (struct sockaddr *)&sender_addr, &sender_addr_len)) > 0)
    {
        printf("Chauffage : %s\n", buffer);
        deserializeMessage(buffer, recvlen, &msg);
        printf("Commande recue : type=%d, valeur=%d, piece=%d\n", msg.type, msg.valeur, msg.piece);
        // Traiter les commandes du chauffage
        Appareil app = rechercher_appareil(msg.piece);
        // envoi
        if (send(app.socket, buffer, recvlen, 0) == SOCKET_ERROR)
        {
            fprintf(stderr, "Erreur d'envoi au chauffage : %d\n", WSAGetLastError());
        }
        else
        {
            printf("Commande envoyée avec succès au chauffage : socket=%d, pièce=%s\n", app.socket, msg.piece);
        }
    }
...
Appareil rechercher_appareil(const char *piece)
{
    ...
    {
        if (strncmp(appareils[i].piece, piece, NOM_MAX) == 0)
        {
            return appareils[i];
        }
    }
    ...
    printf("L'appareil dans la pièce %s n'existe pas !\n", piece);

    // Retourner une structure vide
    Appareil appareil_invalide = {"", INVALID_SOCKET, -1};
    return appareil_invalide;
}
...
void envoyer_udp_securise(SOCKET udp_socket, const char *buffer, size_t length, struct sockaddr_in *addr)
{
    ...
    if (sendto(udp_socket, buffer, length, 0, (struct sockaddr *)addr, sizeof(*addr)) == SOCKET_ERROR)
    {
        fprintf(stderr, "Erreur d'envoi UDP : %d\n", WSAGetLastError());
    }
